...
throughput. A placed timing study isolates the per-lane fused multiply-add as the
frequency bottleneck and, via a design-space exploration showing the limit is
intra-lane rather than fabric-wide, pipelines the FMA into three stages to
\emph{meet} 100~MHz---moving the critical path off the FP datapath entirely
(Section~\ref{sec:fp}).
\end{itemize}
...
area ($\propto\!\mathrm{lanes}\times\mathrm{warps}$); for the FP-enabled design we
pair this with a placed area/frequency study in Section~\ref{sec:fppa}.
...
direct confirmation that interconnect, not logic depth, bounds this design.

\paragraph{Frequency does not scale with fabric size.} If congestion is the limit,
a natural question is whether \emph{trimming} the FP fabric relieves it further. It
does not. Placing the FP design at four lanes (Table~\ref{tab:fpdse}) halves its
area---37k versus 69k LUTs, 20 versus 40 DSPs---but reaches only 79.7~MHz,
marginally \emph{below} the eight-lane 84.1~MHz, and the router still reports CLB
congestion. The reason is that the critical path is a \emph{single} lane's FMA cone,
whose logic depth and locally-dense 128-bit datapath are independent of the lane
count; halving the lanes replicates the same cone fewer times and packs the smaller
design into a denser region that gives the lone FMA no relief. The eight-lane
configuration therefore Pareto-dominates the four-lane one---equal frequency at
twice the throughput---which both explains why the shared-SFU saving helped only
modestly and identifies the true frequency lever as the FMA \emph{microarchitecture}
(a deeper FMA pipeline), not the fabric size. The area and power figures here are
exact.

\paragraph{Closing 100~MHz: a three-stage FMA.} Acting on that finding, we split the
FMA across \emph{three} stages---product, 128-bit align-and-add, and
normalize-and-round---so no single stage carries the whole cone (the two-stage split
had left product, align, and add together in stage one). The warp pool absorbs the
extra cycle with the same single-slot scoreboard, now a two-cycle writeback latency
that SIMT interleaving continues to hide on streaming work; the added pipeline
registers raise flip-flops to 15.0k but leave LUTs, DSPs, and the bit-exact results
unchanged. Place-and-route now \emph{meets} timing at 100~MHz (worst setup slack
$+0.071$~ns; Table~\ref{tab:fpclose} traces $78\rightarrow84\rightarrow100.7$~MHz
across the three closure steps), and decisively the critical path moves \emph{off}
the FMA onto the vector-register-file LUTRAM read mux---the same structure that
bounds the \emph{integer} design (Section~\ref{sec:impl}). The floating-point
datapath is no longer the frequency limiter, confirming that FMA logic depth---the
lever the design-space exploration identified---was the true constraint all along.

\begin{table}[t]
\caption{FP timing closure on \texttt{xczu7ev} (placed, 8 lanes / 4 warps). Pipelining
the FMA from two to three stages moves the critical path off the FMA and meets
100~MHz; the shared SFU and lane trimming alone did not.}
\label{tab:fpclose}
\centering
\footnotesize
\begin{tabular}{llrrrl}
\toprule
Step & FP datapath & Fmax & WNS & FF & Critical path\\
\midrule
1 & 2-stage FMA      & 78.0  & $-2.816$ & 12.3k & FMA cone\\
2 & {}+ shared SFU   & 84.1  & $-1.890$ & 11.2k & FMA cone\\
3 & 3-stage FMA      & \textbf{100.7} & $\mathbf{+0.071}$ & 15.0k & VRF LUTRAM\\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[t]
\caption{FP design-space exploration: placed PPA of the FP-enabled accelerator on
\texttt{xczu7ev} versus lane count (4 warps). Area halves with the lanes but the
placed frequency does not improve---the per-lane FMA cone is the floor.}
\label{tab:fpdse}
\centering
\footnotesize
\begin{tabular}{lrrrrr}
\toprule
Lanes & LUT & DSP & Power & Placed Fmax & Thr.\ (w/cyc)\\
\midrule
4 & 36{,}785 & 20 & 0.98~W & 79.7~MHz & 3.06\\
8 & 68{,}914 & 40 & 1.22~W & 84.1~MHz & 5.85\\
\bottomrule
\end{tabular}
\end{table}
...
in both precisions, and its timing closure is complete: a design-space exploration
localized the bottleneck to a single lane's FMA cone (trimming the fabric did not
help), and pipelining that FMA into three stages meets 100~MHz with the critical
path moved off the FP datapath (Section~\ref{sec:fppa}). The remaining FP headroom
is therefore in the now-binding vector-register-file read mux, shared with the
integer design. The throughput design-space sweep (Section~\ref{sec:dse})
establishes how performance scales with lanes and warps, and the FP PPA sweep
(Section~\ref{sec:fppa}) completes the area/frequency/throughput picture. Finally, an open-source ASIC path through the
