...
The single-cycle unit implements add, subtract, multiply, the fused multiply-add
...
stays single-cycle and commits through the ordinary writeback path. Half-precision
...
mis-NaN-boxed-operand, and conversion corners, the single-cycle unit passes
...
benefit is part of the PPA discussion below.
...
\midrule
\texttt{fsaxpy}   & $\alpha A_i + B_i$ (fmadd) & 393  & 96 & 100\%\\
\texttt{fsaxpy16} & $\alpha A_i + B_i$ (FP16)  & 425  & 96 & 100\%\\
\texttt{fnorm}    & $A_i/\sqrt{B_i}$ (div+sqrt)& 2{,}396 & 96 & 100\%\\
\texttt{fdot}     & $\sum_l A_{8w+l}B_{8w+l}$  & 1{,}587 & 96 & 45\%\\
\bottomrule
\end{tabular}
\end{table}
...
(the FP multipliers), at 1.34~W. The increase is dominated by the
\emph{single-cycle} unit---eight per-lane copies of the add/multiply/FMA/convert
datapath, the fused multiply-add being the largest single block---rather than the
iterative divide/square-root units, which synthesize to only $689$ LUTs each
...
the area; the single-cycle FMA is both the dominant area term and, as a long
combinational chain, the new critical path (a conservative non-timing-driven
estimate puts the raw path at 16.2~ns). Pipelining the FMA---and sharing the
divide/square-root unit at high lane counts---are the clear paths to closing
100~MHz on the FP design, and are left as future work; the area and power figures
here are exact.
...
high lane counts. The throughput design-space sweep (Section~\ref{sec:dse})
