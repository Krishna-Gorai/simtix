...
serial unit that recovered both area and frequency (84~MHz post-route,
Section~\ref{sec:fppa}). The FP design-space exploration then localized the
remaining barrier: trimming the FP fabric does \emph{not} raise the frequency
(four lanes places below eight), because the critical path is a single lane's FMA
cone. The path to 100~MHz is therefore the FMA \emph{microarchitecture}---a deeper,
more finely balanced FMA pipeline---rather than fewer lanes or further area cuts;
that, and an ASIC-flow re-balancing where wire delay is less dominant, are the
clear next steps. The throughput design-space sweep (Section~\ref{sec:dse})
