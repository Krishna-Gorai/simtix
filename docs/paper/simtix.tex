...
divergent kernel). The complete RTL, benchmark harness, and FPGA flow are
publicly available.
\end{abstract}

\begin{IEEEkeywords}
SIMT, GPGPU, RISC-V, hardware accelerator, control divergence, memory
coalescing, FPGA, clock gating, open-source hardware.
\end{IEEEkeywords}
...
savings. The throughput design-space sweep (Section~\ref{sec:dse}) establishes
how performance scales with lanes and warps; the complementary piece in progress
is a per-configuration FPGA and ASIC PPA sweep to complete the
...
on divergent code. The design and its evaluation infrastructure are public, so
