...
digit-recurrence, so they are the engine's first multi-cycle operations. We give
each lane a small iterative unit (restoring division of the significands; a
restoring integer square root of the radicand scaled for the fractional bits) and
park the issuing warp on a \emph{stall scoreboard} while it iterates---the same
mechanism the background memory engine already uses for latency hiding. When a
warp issues a divide or square root, its lanes' units start on the live operands
and the warp enters a waiting state; the round-robin scheduler keeps issuing
\emph{other} warps onto the idle datapath, and the parked warp resumes when its
units complete and write the f-file (the memory engine keeps write-port priority,
so the result waits a cycle if they collide). At most one warp's divide/square
root is in flight, exactly as at most one memory access is. Half precision reuses
the FP32 core by the same widen--compute--narrow argument (the $24\ge2\cdot11+2$
bound holds for divide and square root too). We build one unit \emph{per lane}
here for correctness; because a divide/square root unit is comparatively large,
folding the lanes onto one shared serial unit is an area/throughput trade we leave
to the PPA study (Section~\ref{sec:future}).
...
the binding constraint. Closing 100~MHz therefore calls for reducing FP area and
congestion---the shared serial SFU and the lane/warp design-space exploration of
Section~\ref{sec:dse}---or a third pipeline stage that further subdivides the
product/align cone. The area and power figures here are exact.
...
in both precisions; its one remaining trade is the divide/square-root unit, built
per-lane here for correctness---folding the lanes onto a single shared serial unit
would trade throughput for a large area saving and is the natural next step at
high lane counts. That same area reduction is the lever for FP frequency: the
pipelined FP unit closes 78~MHz post-route and is interconnect-bound in the
enlarged fabric (Section~\ref{sec:fppa}), so shrinking the FP footprint---or
adding a third FMA pipeline stage to subdivide the product/align cone---is the
route to 100~MHz. The throughput design-space sweep (Section~\ref{sec:dse})
