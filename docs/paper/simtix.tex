...
\paragraph{Timing closure.} Table~\ref{tab:fpclose} summarizes the placed
utilization and frequency of each step in the closure; we walk through them in turn.
To recover frequency we first pipeline the FP unit into two
...
(Figure~\ref{fig:fpthr}). The pipeline registers raise flip-flops to
12.3k---the registered datapath is wide, eight lanes each holding a 128-bit
accumulator---but leave LUTs and DSPs essentially unchanged (Table~\ref{tab:fpclose},
step~1). Timing-driven
...
does not. Placing the FP design at four lanes (Table~\ref{tab:fpclose}, last row)
halves its area---37k versus 69k LUTs, 20 versus 40 DSPs---but reaches only 79.7~MHz,
...
that SIMT interleaving continues to hide on streaming work; the added third-stage
registers raise flip-flops to 15.0k (Table~\ref{tab:fpclose}) while leaving LUTs
(marginally up, to 70.0k), DSPs, power (1.22~W), and the bit-exact results unchanged.
Place-and-route now \emph{meets} timing at 100~MHz (worst setup slack
