...
| **M13** | Post-implementation functional sim: simulate the routed gate-level netlist ([docs/m13_postimpl_sim.md](m13_postimpl_sim.md)) | The real placed-and-routed netlist (UNISIM `LUT6`/`FDRE`/`RAMD64E`/`DSP48E2` + `glbl` GSR) simulated in xsim with the same self-checking testbench → **result=964**, identical to RTL. Sign-off that synth + P&R preserved behaviour on actual hardware | ✅ done |
